...
skill.  In later runs, weights are exported frequently (not only on checkpoint
intervals), paths are resolved to absolute locations for Ray workers, and
action selection uses temperature sampling ($\tau \approx 0.8$) instead of
...
RTX~5090 specifically.  Production league runs use \texttt{pure\_league\_play}
